...
contract('Settlement', accts => {
  before(async () => {
    ...
    const TokenA = Token.new(Tokens[0].name, Tokens[0].symbol, Tokens[0].decimals, Tokens[0].supply)
    const TokenB = Token.new(Tokens[1].name, Tokens[1].symbol, Tokens[1].decimals, Tokens[1].supply)
